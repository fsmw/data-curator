...
class OWIDSearcher:
    ...
    def search(self, query: str, max_results: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for chart in data.get("results", []):
                # Convert OWID chart format to our standard indicator format
                indicator = {
                    "id": f"owid_{chart['slug']}",  # Unique ID with prefix
                    ...
                    "available_countries": chart.get("availableEntities", [])[:10],  # First 10 countries
    ...
    def _extract_tags(self, chart: Dict[str, Any]) -> List[str]:
        ...
        keywords = ["gdp", "tax", "revenue", "income", "wage", "employment", 
                   "unemployment", "inflation", "trade", "poverty", "inequality"]
